...
function starBazFunc(node) {
  ...
  window.appBus
    .filter(function (msg) { return msg.tell == 'favsUpdate'})
    .map(function (msg) { return msg.favs })
    .toProperty(function () { return {} })
    .onValue(function (favs) {
      favs[getAttrs(node).id] ? node.setAttribute('data-star-active', 1) : node.removeAttribute('data-star-active')
    })
}
